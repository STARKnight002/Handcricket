...
def choice():
    global ch
    ch = input('Type your choice: BAT or BOWL - ')
    while True:
        if ((ch.lower()) == 'bat') or ((ch.lower()) == 'bowl'):
            break
        else:
            print("Enter 'bat' or 'bowl' only")
...
def end():
    ...
    time.sleep(0.5)
    print("Done")
    pl = input("""Type 'play' if you want to restart innings. 
Hit ENTER otherwise - """)
    if pl.lower()=='play':
        run()
    else:
